...
def test_should_raise_exception_when_tmdb_actor_not_found(
    mocker: MockerFixture, resource_id
):
    ...
    with pytest.raises(NotFound) as ex:
        TmdbService().fetch_actor(resource_id)

    assert ex.value.detail["detail"] == f"Tmdb actor with id {resource_id} not found"


def test_should_fetch_tmdb_actor(mocker: MockerFixture, tmdb_actor: bytes, resource_id):
    mock_response: MagicMock = MagicMock()
    mock_response.status_code = 200
    mock_response.content = tmdb_actor
    mocker.patch("requests.get", return_value=mock_response)

    result: TmdbActorResponse = TmdbService().fetch_actor(resource_id)

    assert result.name == "Christian Bale"
    assert result.imdb_id == "nm0000288"


def test_should_fetch_tmdb_movie(mocker: MockerFixture, tmdb_movie: bytes, resource_id):
    mock_response: MagicMock = MagicMock()
    mock_response.status_code = 200
    mock_response.content = tmdb_movie
    mocker.patch("requests.get", return_value=mock_response)

    result: TmdbMovieResponse = TmdbService().fetch_movie(resource_id)
...
def test_should_raise_exception_when_tmdb_movie_not_found(
    ...
    with pytest.raises(NotFound) as ex:
        TmdbService().fetch_movie(resource_id)

    assert ex.value.detail["detail"] == f"Tmdb movie with id {resource_id} not found"


def test_should_fetch_movie_trailers(
    mocker: MockerFixture, tmdb_movie_trailer: bytes, resource_id
):
    mock_response: MagicMock = MagicMock()
    mock_response.status_code = 200
...
def test_fetch_movie_credits(
    mocker: MockerFixture, tmdb_movie_credits: bytes, resource_id
):
    mock_response: MagicMock = MagicMock()
    mock_response.status_code = 200
    ...
    result: TmdbMovieCreditsResponse = TmdbService().fetch_movie_credits(resource_id)
